...
function handleiOSTouch(ev) {
  if (Graphics._video.paused && Graphics.isVideoPlaying())
...
Graphics.isVideoPlaying = function () {
  if (Utils.isMobileSafari())
    return waitForLoading || (this._video && this._isVideoVisible());
...
ShaderTilemap.prototype._hackRenderer = function (renderer) {
  ...
  renderer.plugins.tile.tileAnim[1] =
...
WindowLayer.prototype._maskWindow = function (window, shift) {
  ...
  rect.y =
    shift.y + window.y + (window.height / 2) * (1 - window._openness / 255);
