...
(function () {
  ...
  var getArgNumber = function (arg, min, max) {
    ...
    return (parseInt(convertEscapeCharacters(arg.toString())) || 0).clamp(
      min,
      max
    );
  };
  ...
  Game_Interpreter.prototype.pluginCommandDTextPicture = function (
    command,
    args
  ) {
    switch (getCommandName(command)) {
        ...
        switch (getCommandName(args[0])) {
          case "ALIGN":
            $gameScreen.dTextAlign = isNaN(args[1])
              ? Game_Interpreter.textAlignMapper[getArgString(args[1], true)]
      ...
      case "D_TEXT_WINDOW_CURSOR_ACTIVE":
        $gameScreen.setDTextWindowCursorActive(
          getArgNumber(args[0], 0),
  ...
  Game_Screen.prototype.setDTextPicture = function (value, size) {
    ...
    this.dUsingVariables = (this.dUsingVariables || []).concat(
      getUsingVariables(value)
    );
    this.dTextValue =
      (this.dTextValue || "") + getArgString(value, false) + "\n";
  ...
  Game_Screen.prototype.setDTextWindowCursorActive = function (
    pictureId,
    value
  ) {
    var picture = this.picture(pictureId);
  ...
  Game_Picture.prototype.updateDTextVariable = function () {
    $gameVariables.getChangedVariables().forEach(function (variableId) {
      if (this.dTextInfo.usingVariables.contains(variableId)) {
        ...
        this.dTextInfo.value = getArgString(
          this.dTextInfo.originalValue,
          false
        );
      }
    }, this);
  ...
  Window_Base.prototype.convertEscapeCharacters = function (text) {
    ...
    text = text.replace(
      /\x1bV\[(\d+),\s*(\d+)]/gi,
      function () {
        return this.getVariablePadZero(
          $gameVariables.value(parseInt(arguments[1], 10)),
          arguments[2]
        );
      }.bind(this)
    );
    ...
    text = text.replace(
      ...
      function () {
        var item =
          $dataWeapons[getArgNumber(arguments[1], 1, $dataWeapons.length)];
        return this.getItemInfoText(item);
      }.bind(this)
    );
    text = text.replace(
      ...
      function () {
        var item =
          $dataArmors[getArgNumber(arguments[1], 1, $dataArmors.length)];
        return this.getItemInfoText(item);
      }.bind(this)
    );
    text = text.replace(
      ...
      function () {
        var item =
          $dataSkills[getArgNumber(arguments[1], 1, $dataSkills.length)];
        return this.getItemInfoText(item);
      }.bind(this)
    );
    text = text.replace(
      ...
      function () {
        var item =
          $dataStates[getArgNumber(arguments[1], 1, $dataStates.length)];
        return this.getItemInfoText(item);
      }.bind(this)
    );
    return text;
  ...
  Window_Base.prototype.getItemInfoText = function (item) {
    ...
    return (
      (this.isValidDTextIconSwitch() ? "\x1bi[" + item.iconIndex + "]" : "") +
      item.name
    );
  };
  ...
  Window_Base.prototype.isValidDTextIconSwitch = function () {
    return (
      !param.itemIconSwitchId || $gameSwitches.value(param.itemIconSwitchId)
    );
  };
  ...
  Sprite_Picture.prototype.updateFrameWindow = function () {
    ...
    this._frameWindow.x =
      this.x - this.anchor.x * this.width * this.scale.x - padding;
    this._frameWindow.y =
      this.y - this.anchor.y * this.height * this.scale.y - padding;
  ...
  Sprite_Picture.prototype.adjustScaleFrameWindow = function () {
    ...
    if (
      this._frameWindow.width !== newFrameWidth ||
      this._frameWindow.height !== newFrameHeight
    ) {
      this._frameWindow.move(
        this._frameWindow.x,
        this._frameWindow.y,
        newFrameWidth,
        newFrameHeight
  ...
  Sprite_Picture.prototype.makeDynamicBitmap = function () {
    ...
    if (this.dTextInfo.windowFrame) {
      ...
      this.makeFrameWindow(
        bitmapVirtual.width * scaleX,
        bitmapVirtual.height * scaleY
      );
    }
  ...
  Sprite_Picture.prototype.applyTextDecoration = function () {
    if (textDecParam.Mode >= 0) {
      this.bitmap.outlineColor = "rgba(%1,%2,%3,%4)".format(
        textDecParam.Red,
        textDecParam.Green,
        textDecParam.Blue,
        textDecParam.Alpha / 255
      );
  ...
  Sprite_Picture.prototype.makeFrameWindow = function (width, height) {
    ...
    this._frameWindow = new Window_Base(
      0,
      0,
      width + padding * 2,
      height + padding * 2
  ...
  Sprite_Picture.prototype._processCharacter = function (textState, bitmap) {
    if (textState.text[textState.index] === "\x1b") {
      ...
      switch (code) {
        case "C":
          bitmap.textColor = this.hiddenWindow.textColor(
            this.hiddenWindow.obtainEscapeParam(textState)
          );
          break;
        case "I":
          this._processDrawIcon(
            this.hiddenWindow.obtainEscapeParam(textState),
            textState,
            bitmap
          );
        ...
        case "F":
          switch (
            this.hiddenWindow.obtainEscapeParamString(textState).toUpperCase()
          ) {
            case "I":
  ...
  Sprite_Picture.prototype._processNewLine = function (textState, bitmap) {
    ...
    if (bitmap instanceof Bitmap)
      textState.x =
        ((bitmap.width - this.textWidths[textState.line]) / 2) *
        this.dTextInfo.align;
  };

  Sprite_Picture.prototype._processDrawIcon = function (
    iconIndex,
    textState,
    bitmap
  ) {
    ...
    bitmap.blt(
      ...
      ph,
      textState.x + 2,
      textState.y + (textState.height - ph) / 2
    );
  ...
  Bitmap_Virtual.prototype.blt = function (
    source,
    sx,
    sy,
    sw,
  ...
  Bitmap.prototype._makeFontNameText = function () {
    return (
      (this.fontBoldFotDtext ? "bold " : "") +
      _Bitmap__makeFontNameText.apply(this, arguments)
    );
  };
})();
