...
(function ($) {
  $.Params = $plugins.filter(function (p) {
    return p.description.contains("<TTKC ItemsHUD>");
  })[0].parameters;
  ...
  Window_Items_HUD.prototype.update = function () {
    ...
    if (newValues.join() !== $.lastItemsValues.join() || this._firstDraw) {
      ...
      this.width =
        lastSpace +
        40 +
        this.textWidth(newValues[0]) +
        this.textWidth(newValues[newValues.length - 1]);
      this._lastPos = null;
    ...
    if (this._lastPos != $.position) this.setPosition();
  };
    ...
    if (command == "ItemsHUD") {
      switch (args[0]) {
        ...
        case "AddItem":
          ...
          if (!~$.items.indexOf(itemId)) $.items.push(itemId);
          break;
