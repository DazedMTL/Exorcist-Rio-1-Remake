...
TMPlugin.MapHpGauge.WindowX = +(
  TMPlugin.MapHpGauge.Parameters["gaugeWindowX"] || 0
);
TMPlugin.MapHpGauge.WindowY = +(
  TMPlugin.MapHpGauge.Parameters["gaugeWindowY"] || 0
);
TMPlugin.MapHpGauge.WindowWidth = +(
  TMPlugin.MapHpGauge.Parameters["gaugeWindowWidth"] || 288
);
TMPlugin.MapHpGauge.WindowHeight = +(
  TMPlugin.MapHpGauge.Parameters["gaugeWindowHeight"] || 64
);
...
["A", "B", "C", "D"].forEach(function (code) {
  TMPlugin.MapHpGauge["Gauge" + code] = {
    ...
    fontSize: +(
      TMPlugin.MapHpGauge.Parameters["gauge" + code + "FontSize"] || 28
    ),
    ...
    color: (
      TMPlugin.MapHpGauge.Parameters["gauge" + code + "Color"] ||
      "#ff60c0 #ffa0e0"
    ).split(" "),
  };
  TMPlugin.MapHpGauge.Gauges.push(TMPlugin.MapHpGauge["Gauge" + code]);
});
TMPlugin.MapHpGauge.FaceOffsetX = +(
  TMPlugin.MapHpGauge.Parameters["faceOffsetX"] || -4
);
TMPlugin.MapHpGauge.FaceOffsetY = +(
  TMPlugin.MapHpGauge.Parameters["faceOffsetY"] || -4
);
TMPlugin.MapHpGauge.StateIconMax = +(
  TMPlugin.MapHpGauge.Parameters["stateIconMax"] || 4
);
TMPlugin.MapHpGauge.StateIconX = +(
  TMPlugin.MapHpGauge.Parameters["stateIconX"] || 156
);
TMPlugin.MapHpGauge.StateIconY = +(
  TMPlugin.MapHpGauge.Parameters["stateIconY"] || 24
);
TMPlugin.MapHpGauge.GoldWidth = +(
  TMPlugin.MapHpGauge.Parameters["goldWidth"] || 0
);
...
TMPlugin.MapHpGauge.ShakeTime = +(
  TMPlugin.MapHpGauge.Parameters["shakeTime"] || 20
);
TMPlugin.MapHpGauge.CollideOpacity = +(
  TMPlugin.MapHpGauge.Parameters["collideOpacity"] || 128
);
TMPlugin.MapHpGauge.StartVisible =
  TMPlugin.MapHpGauge.Parameters["startVisible"] === "1";
TMPlugin.MapHpGauge.WindowOpacity = +(
  TMPlugin.MapHpGauge.Parameters["windowOpacity"] || 255
);
TMPlugin.MapHpGauge.MessageBusyHide =
  TMPlugin.MapHpGauge.Parameters["messageBusyHide"] === "1";
TMPlugin.MapHpGauge.EventBusyHide =
  TMPlugin.MapHpGauge.Parameters["eventBusyHide"] === "1";
...
(function () {
    ...
    if (this.updateVisibility()) {
      ...
      if (needRefresh) {
        ...
        if (needRefresh === "SHAKE")
          this._shakeDuration = TMPlugin.MapHpGauge.ShakeTime;
        for (var i = 0; i < TMPlugin.MapHpGauge.Gauges.length; i++) {
          this._gaugeVisible[i] = $gameSystem.isVisibleMapHpGauges(i);
  ...
  Window_MapHpGauge.prototype.updateVisibility = function () {
    ...
    if (
      (TMPlugin.MapHpGauge.EventBusyHide && $gameMap.isEventRunning()) ||
  ...
  Window_MapHpGauge.prototype.isNeedRefresh = function () {
    ...
    if (actor) {
      ...
      if (TMPlugin.MapHpGauge.GoldWidth > 0 && this._gold !== $gameParty.gold())
        return true;
    }
    return false;
  };
    ...
    if (actor) {
      ...
      if (TMPlugin.MapHpGauge.GoldWidth > 0) {
        this.drawCurrencyValue(
          ...
          TMPlugin.MapHpGauge.GoldX,
          TMPlugin.MapHpGauge.GoldY,
          TMPlugin.MapHpGauge.GoldWidth
        );
      }
      this._lineHeight = 36;
  ...
  Window_MapHpGauge.prototype.refreshFace = function () {
    ...
    if (!this._needFaceRefresh) {
      var x =
        TMPlugin.MapHpGauge.WindowWidth - 144 + TMPlugin.MapHpGauge.FaceOffsetX;
