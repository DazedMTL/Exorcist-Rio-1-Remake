...
(function () {
  ...
  var getArgNumber = function (arg, min, max) {
    ...
    return (
      parseInt(convertEscapeCharacters(arg), 10) ||
      makeRandomCompatible[arg] ||
      0
    ).clamp(min, max);
  };
  ...
  Game_Interpreter.prototype.pluginCommandEventReSpawn = function (
    command,
    args,
    extend
  ) {
    switch (getCommandName(command)) {
      ...
      case "MAKE":
        ...
        $gameMap.spawnEvent(
          this.getEventIdForEventReSpawn(args[0], extend),
          x,
          y,
          extend
        );
  ...
  Game_Interpreter.prototype.getEventIdForEventReSpawn = function (
    arg,
    isTemplate
  ) {
    var id = 0;
    if (!isNaN(convertEscapeCharacters(arg))) {
      ...
      var event = DataManager.searchDataItem(
        isTemplate ? $dataTemplateEvents : $dataMap.events,
        "name",
        arg
      );
      id = event ? event.id : 0;
  ...
  Game_Map.prototype.spawnEvent = function (originalEventId, x, y, isTemplate) {
    if (
      this.isExistEventData(originalEventId, isTemplate) &&
      $gameMap.isValid(x, y)
    ) {
      var eventId = this.getEventIdSequence();
      var event = new Game_PrefabEvent(
        this._mapId,
        eventId,
        originalEventId,
        x,
  ...
  Game_Map.prototype.spawnEventRandom = function (
    originalEventId,
    conditionMap,
    isTemplate
  ) {
    ...
    if (conditionMap.screen) {
      conditions.push(
        this.isErsCheckScreenInOut.bind(this, conditionMap.screen)
      );
    }
    if (conditionMap.collided) {
      conditions.push(
        this.isErsCheckCollidedSomeone.bind(this, conditionMap.collided)
      );
    }
    if (conditionMap.terrainTag) {
      conditions.push(
        this.isErsCheckTerrainTag.bind(this, conditionMap.terrainTag)
      );
    }
    if (conditionMap.regionId) {
      conditions.push(
        this.isErsCheckRegionId.bind(this, conditionMap.regionId)
      );
    }
  ...
  Game_Map.prototype.isSameMapReload = function () {
    return (
      !$gamePlayer.isTransferring() || this.mapId() === $gamePlayer.newMapId()
    );
  };
  ...
  Game_Map.prototype.getConditionalValidPosition = function (conditions) {
    ...
    do {
      ...
      !conditions.every(this.checkValidPosition.bind(this, x, y)) &&
      ++count < 1000
    );
  ...
  Game_Map.prototype.isErsCheckScreenInOut = function (type, x, y) {
    return type === 1
      ? this.isErsCheckInnerScreen(x, y)
      : this.isErsCheckOuterScreen(x, y);
  };
  ...
  Game_Map.prototype.isErsCheckInnerScreen = function (x, y) {
    ...
    return (
      ax >= 0 &&
      ay >= 0 &&
      ax <= this.screenTileX() - 1 &&
      ay <= this.screenTileY() - 1
    );
  };
  ...
  Game_Map.prototype.isErsCheckOuterScreen = function (x, y) {
    ...
    var ay = this.adjustY(y);
    return (
      ax < -1 || ay < -1 || ax > this.screenTileX() || ay > this.screenTileY()
    );
  };
  ...
  Game_PrefabEvent.prototype.initialize = function (
    mapId,
    eventId,
    originalEventId,
    x,
  ...
  Game_PrefabEvent.prototype.linkEventData = function () {
    $dataMap.events[this._eventId] = this._isTemplate
      ? $dataTemplateEvents[this._originalEventId]
      : $dataMap.events[this._originalEventId];
  };
  ...
  Spriteset_Map.prototype.updatePrefabEvent = function () {
    $gameMap.getPrefabEvents().forEach(
      function (event) {
