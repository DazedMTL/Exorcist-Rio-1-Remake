...
(function () {
      ...
      switch (type) {
        ...
        case "switch":
          ...
          if (!value.match(/^([A-D]|\d+)$/i)) {
            throw new Error(
              "Plugin parameter value is not switch : " + param + " : " + value
            );
          }
          break;
        default:
  ...
  Game_System.prototype.setSwitchStatuses = function (sw, eventId) {
    if (this._switchStatuses[sw]) {
      if (
        this._switchStatuses[sw] instanceof Array &&
  ...
  var _Game_CharacterBaseMoveStraight =
  ...
  var _Game_CharacterBaseMoveDiagonally =
  ...
  var _Game_CharacterBaseSetDirection =
  ...
  Game_CharacterBase.prototype.isMapPassableEx = function (x, y, d) {
    ...
    regionDecisions = getRegionIds(
      DefRegionDecisions,
      this.getRegionDecision()
    );
    passableFlag = true;

    if ($gameMap.isPassable(x, y, d) && $gameMap.isPassable(x2, y2, d2)) {
      if (
        this.getEventDecision() == 1 ||
  ...
  Game_Event.prototype.setupSensor = function () {
    var event, pattern, match, note, cnt, i, n, m, options, op, value;
    event = this.event();
    pattern =
      /<(.?)(?:psensor)(l|f|s|d)?(?:\:)(\\v\[\d+\]|\d+)([ 0-9a-z\[\]\\]*)?>/i;
...
*/
  ...
  Game_Event.prototype.isSideSearch = function (
    directionR,
    directionL,
    vx,
    vy
  ...
  Game_Event.prototype.rangeSearch = function (
    strDir,
    rx,
    ry,
    signX,
    ...
    for (j = 0; j <= cnt; j++) {
      ...
      if (
        ...
        if (
          !this.isMapPassableEx(ex + cx, ey + cy, strDir) &&
          j < sensorRange
        ) {
  ...
  Sprite_ViewRange.prototype.updateBitmap = function () {
    ...
    cnt =
      tmpCoordinate.length < coordinate.length
        ? tmpCoordinate.length
        : coordinate.length;
    bias = bothSensor ? 3 : this._character.getBothSensor() > 0 ? 3 : 1;
    color = DefRangeColor[0];
    opacity = DefRangeOpacity[0];
    ...
    for (i = 0; i < cnt; i++) {
      if (
        coordinate[i][0] != tmpCoordinate[i][0] ||
        coordinate[i][1] != tmpCoordinate[i][1]
      ) {
  ...
  Bitmap.prototype.fillViewRangeLine = function (color, character) {
    ...
    if (direction == DIR_UP) {
      if (coordinates && cnt == 1) {
        ...
        this.drawSideLine(context, cx, cy, [
          sideSensorR,
          1,
          1,
          sideSensorL,
  ...
  Bitmap.prototype.fillViewRangeFan = function (color, character) {
    ...
    if (direction == DIR_UP) {
      if (coordinates && cnt > 0) {
        ...
        distanceX = cx - tileWidth;
        distanceY =
          height - tileHeight - Math.abs(coordinates[0][num]) * tileHeight;
        this.drawSideLine(context, cx, cy, [
          sideSensorR,
          1,
          1,
        ...
        for (i = 1, j = 2; j < cnt; i++, j++) {
          ...
          distanceY =
            height - tileHeight - Math.abs(coordinates[j][num]) * tileHeight;
      ...
      if (coordinates && cnt > 0) {
        ...
        cy = height / 2 - tileHeight / 2;
        distanceX =
          width - tileWidth - Math.abs(coordinates[0][num]) * tileWidth;
        distanceY = cy + tileHeight;
        this.drawSideLine(context, cx, cy, [
          sideSensorR,
          1,
        ...
        for (i = 1, j = 2; j < cnt; i++, j++) {
          ...
          cy = height / 2 + (tileHeight / 2) * sign + tileHeight * i * sign;
          distanceX =
            width - tileWidth - Math.abs(coordinates[j][num]) * tileWidth;
          distanceY = cy + tileHeight * signChange(sign);
