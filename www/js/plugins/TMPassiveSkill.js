 ...
 *
 * @help
 * TMPlugin - パッシヴスキル ver1.1.0
 *
 * 使い方:
 *
 *   スキルのメモ欄に <passive:50> のようなタグを使って設定します。
 *   この例ではスキルを習得している間、50 番の武器を装備したときと
 *   同じ効果が得られます。
 *
 ...
 *   除外することができます。
 *
 *   パッシヴ効果はスキルだけでなく装備品にも設定することもできます。
 */
...
TMPlugin.PassiveSkill = {};
TMPlugin.PassiveSkill.Parameters = PluginManager.parameters("TMPassiveSkill");
TMPlugin.PassiveSkill.PassiveCommands = TMPlugin.PassiveSkill.Parameters[
  "passiveCommands"
]
  .split("")
  .map(Number);
...
(function () {
  ...
  Game_Actor.prototype.isPassiveSkillTurnOk = function (item) {
    if (item.meta.passiveTurn) {
      return (
        $gameParty.inBattle() &&
        $gameTroop.turnCount() === +item.meta.passiveTurn
      );
    }
    return true;
  };
  ...
  Game_Actor.prototype.addedSkillTypes = function () {
    ...
    if ($gameParty.inBattle()) {
      for (var i = 0; i < skillTypes.length; i++) {
        if (TMPlugin.PassiveSkill.PassiveCommands.contains(skillTypes[i])) {
