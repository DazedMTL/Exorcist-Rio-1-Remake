...
(function () {
  ...
  Params.foregroundColor =
    pluginParams["Foreground color"] || "rgba(224, 224, 240, 0.6)";
  Params.backgroundColor =
    pluginParams["Background color"] || "rgba(  0,   0, 160, 0.6)";
   ...
   */
  var fillRegularPolygon = function (
    graphics,
    x,
    y,
    width,
   ...
   * ブロックチェック済みフラグの設定
   */
  Game_MinimapPassageCache.prototype.setBlockChecked = function (
    bx,
    by,
    checked
  ) {
  ...
  Game_MinimapPassageCache.prototype.isPassableDir = function (x, y, dir) {
    ...
    switch (dir) {
      case 2:
        return (flag & Minimap.dirFlags.down) !== 0;
      case 4:
        return (flag & Minimap.dirFlags.left) !== 0;
      case 6:
        return (flag & Minimap.dirFlags.right) !== 0;
      case 8:
        return (flag & Minimap.dirFlags.up) !== 0;
      default:
        return false;
    }
  };
  ...
  Sprite_Minimap.prototype.initialize = function () {
    ...
    this._drawGridNumber = {
      x: this._gridNumber.x + 2,
      y: this._gridNumber.y + 2,
    };
    this._drawRange = {
      begin: new PIXI.Point(0, 0),
  ...
  Sprite_Minimap.prototype.createPassageSprite = function () {
    this._passageSprite = new Sprite_MinimapPassage(
      this._bitmapSize.width,
      this._bitmapSize.height
    );
  ...
  Sprite_Minimap.prototype.updateTilemap = function () {
    ...
    this._tilemap.origin.x =
      ($gamePlayer._realX - this._tileCount.x / 2.0 + 1.5) *
      $gameMap.tileWidth();
    this._tilemap.origin.y =
      ($gamePlayer._realY - this._tileCount.y / 2.0 + 1.5) *
      $gameMap.tileHeight();
  };
  ...
  Sprite_Minimap.prototype.refreshTilemap = function () {
    ...
    var baseSize = {
      ...
      height: Math.floor(
        (rect.height * $gameMap.tileHeight()) / Params.gridSize
      ),
    };
    ...
    (Graphics.isWebGL() ? ShaderTilemap : Tilemap).prototype.update.call(
      this._tilemap
    );
  ...
  Sprite_Minimap.prototype.drawMapForegroundGrid = function (bitmap, x, y) {
    ...
    if (!passage.isPassableDir(x, y, 2)) {
      // 下方向移動不可
      dh -= 1;
    }
    if (!passage.isPassableDir(x, y, 4)) {
      // 左方向移動不可
      dx += 1;
      dw -= 1;
    }
    if (!passage.isPassableDir(x, y, 6)) {
      // 右方向移動不可
      dw -= 1;
    }
    if (!passage.isPassableDir(x, y, 8)) {
      // 上方向移動不可
      dy += 1;
      dh -= 1;
    }
  ...
  Sprite_MinimapIcon.prototype.updateBlink = function () {
    ...
    var alpha =
      128 *
      (Math.sin((this._blinkDuration * Math.PI * 2) / Params.blinkTime) + 1);
  ...
  var _KMS_Spriteset_Map_createUpperLayer =
