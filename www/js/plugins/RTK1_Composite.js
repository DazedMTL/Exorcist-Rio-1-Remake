...
function Scene_CompositeMenu() {
  this.initialize.apply(this, arguments);
}
function Window_CompositeIndex() {
  this.initialize.apply(this, arguments);
}
function Window_CompositeStatus() {
  this.initialize.apply(this, arguments);
}
function Window_RTK_SingleCommand() {
  this.initialize.apply(this, arguments);
}
...
(function (_global) {
  ...
  if (RTK.VERSION_NO < 1.11) {
    throw new Error(
      "This plugin requires version 1.11 or later of RTK1_Core plugin. the current version looks " +
        RTK.VERSION_NO +
        "."
    );
  }
  ...
  M.id4list = function (_mode, _targetList, args, _isObject) {
    ...
    if (args.length == 2) {
      ...
      if (a) {
        var list = args[2].split(",").map(function (o) {
          return a[1] + String(o);
        });
  ...
  M.command = function (args) {
    if (args[0] == "learn") {
      ...
      var a = M._learn.filter(function (o) {
        return (
          (t.contains("item") && o.startsWith("i")) ||
          (t.contains("weapon") && o.startsWith("w")) ||
          (t.contains("armor") && o.startsWith("a"))
        );
      });
      ...
      var t = args[1] == "all" || !args[1] ? "item,weapon,armor" : args[1];
      if (t.contains("item")) {
        M._list = M._list.concat($dataItems.filter(filter_meta));
      }
      if (t.contains("weapon")) {
        M._list = M._list.concat($dataWeapons.filter(filter_meta));
      }
      if (t.contains("armor")) {
        M._list = M._list.concat($dataArmors.filter(filter_meta));
      }
    } else if (args[0] == "clear") {
      var t = args[1] == "all" || !args[1] ? "item,weapon,armor" : args[1];
      M._list = M._list.filter(function (o) {
        return (
          (!t.contains("item") && DataManager.isItem(o)) ||
          (!t.contains("weapon") && DataManager.isWeapon(o)) ||
          (!t.contains("armor") && DataManager.isArmor(o))
        );
      });
    } else if (args[0] == "shop") {
      if (args[1] != "custom") {
      ...
      if (args[2]) {
        ...
        M._Title = RTK.text(
          (args[1] == "all" || !args[1] ? "" : RTK.ucfirst(args[1], " ")) +
            "Composite Shop"
        );
      }
      M._mode = 0;
  ...
  Scene_Menu.prototype.createCommandWindow = function () {
    ...
    this._commandWindow.setHandler(
      "RTK_CompositeCommand",
      this.RTK_CompositeMenu.bind(this)
    );
  };
  ...
  Scene_CompositeMenu.prototype.initialize = function () {
  ...
  Scene_CompositeMenu.prototype.create = function () {
    ...
    if (_chargeable()) {
      ...
      this._goldWindow.move(
        Graphics.boxWidth - this._goldWindow.width,
        0,
    ...
    this._helpWindow.move(
      0,
      0,
      Graphics.boxWidth - gw,
    ...
    this._exeWindow = new Window_RTK_SingleCommand(0, 0, 1, [
      RTK.text("Execute"),
    ]);
    this._exeWindow.move(
      Graphics.boxWidth - 145 - this._exeWindow.textPadding(),
      Graphics.boxHeight -
        this._exeWindow.height -
        this._exeWindow.textPadding(),
      145,
      this._exeWindow.height
    );
    this._exeWindow._funcOkSound = function () {};
    this._exeWindow.setHandler(
      "cancel",
      function () {
    ...
    this._resultWindow = new Window_RTK_SingleCommand(0, 0, 4, [
      "",
      "",
      "DONE",
      "",
    ]);
    this._resultWindow.move(
      Graphics.boxWidth / 4,
      Graphics.boxHeight * 0.42,
      Graphics.boxWidth / 2,
      this._resultWindow.height
    );
  ...
  Window_CompositeStatus.prototype.refresh = function () {
    ...
    if (item[NK]) {
      ...
      if (item[NK].v1[2] < item[NK].v1[1]) {
        this.changePaintOpacity(false);
      }
      this.drawItemName(RTK.id2object(item[NK].v1[0]), x, y);
      this.drawText(
        item[NK].v1[2] + "/" + item[NK].v1[1],
        x * 2,
        y,
        60,
        "right"
      ...
      if (_chargeable()) {
        if ($gameParty.gold() < item[NK].charge) {
          this.changePaintOpacity(false);
        }
        this.changeTextColor(this.systemColor());
        ...
        this.resetTextColor();
        this.drawText(
          item[NK].charge > 0 ? item[NK].charge : "-",
          this.textPadding() + 50,
          y,
          120,
          "right"
      ...
      if (item[NK].v2) {
        if (item[NK].v2[2] < item[NK].v2[1]) {
          this.changePaintOpacity(false);
        }
        this.drawItemName(RTK.id2object(item[NK].v2[0]), x, y);
        this.drawText(
          item[NK].v2[2] + "/" + item[NK].v2[1],
          x * 2,
          y,
          60,
          "right"
      ...
      if (item[NK].v3) {
        if (item[NK].v3[2] < item[NK].v3[1]) {
          this.changePaintOpacity(false);
        }
        this.drawItemName(RTK.id2object(item[NK].v3[0]), x, y);
        this.drawText(
          item[NK].v3[2] + "/" + item[NK].v3[1],
          x * 2,
          y,
          60,
          "right"
      ...
      if (item[NK].v4) {
        if (item[NK].v4[2] < item[NK].v4[1]) {
          this.changePaintOpacity(false);
        }
        this.drawItemName(RTK.id2object(item[NK].v4[0]), x, y);
        this.drawText(
          item[NK].v4[2] + "/" + item[NK].v4[1],
          x * 2,
          y,
          60,
          "right"
      ...
      if (item[NK].v5) {
        if (item[NK].v5[2] < item[NK].v5[1]) {
          this.changePaintOpacity(false);
        }
        this.drawItemName(RTK.id2object(item[NK].v5[0]), x, y);
        this.drawText(
          item[NK].v5[2] + "/" + item[NK].v5[1],
          x * 2,
          y,
          60,
          "right"
  ...
  Window_RTK_SingleCommand.prototype = Object.create(
  ...
  Window_RTK_SingleCommand.prototype.maxCols = function () {
    return 1;
  };
  Window_RTK_SingleCommand.prototype.maxItems = function () {
    return this._line;
  };
